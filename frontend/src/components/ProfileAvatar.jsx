...
const ProfileAvatar = ({user, t, brokeStartAt=100, breakingSteps=10}) => {
    ...
    return (
        <div className="w-full flex justify-center items-center flex-col relative">
            ...
            {pfpIsBroken && <ConfettiExplosion colors={['green', 'light-green', 'white']} particleCount={50} />}
            <Avatar src={pfpIsBroken ? '/transparent.png' : getUserPfp(user)} className={`${pfpIsBroken ? "" : "cursor-pointer"} ${pfpIsExploded ? "pfp-is-egg" : ""}`} onClick={handleClickOnPfp} loading="lazy" variant="circular" alt={user?.username} size="xxl"/>
            {pfpStep !== 0 && (
